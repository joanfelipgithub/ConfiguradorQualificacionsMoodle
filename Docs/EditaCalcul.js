...
const formulas=[];if(Act_Fet_NoFet_RAn.length>0){const numCols=Act_Fet_NoFet_RAn[0].length;for(let i=0;i<numCols;i++){const colValues=Act_Fet_NoFet_RAn.map(row=>row[i]).filter(v=>v);if(colValues.length>0){formulas.push(`RA${i+1}: =average(${colValues.join('+')})/2*10`);}}}
